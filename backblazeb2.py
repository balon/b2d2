...
class BackBlazeB2(object):
    ...
    def authorize_account(self):
        ...
        basic_auth_string = str('Basic ' + (base64.b64encode(id_and_key.encode('utf-8')).decode('utf-8')))
    ...
    def _api_request(self, url, data, headers):
        ...
        request = urllib2.Request(url, encdata, headers)
        response = urllib2.urlopen(request)
        response_data = json.loads(response.read())
        response.close()
        return response_data
